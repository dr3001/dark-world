...
func _process(delta):
	...
	match current_state:
		State.IDLE: _idle(delta, dist_to_player)
		State.PATROL: _patrol(delta, dist_to_player)
		State.CHASE: _chase(delta, dist_to_player)
		State.ATTACK: _attack(delta, dist_to_player)

func _idle(delta, dist):
	idle_timer -= delta
	rotation.y += delta * 0.2
	if dist < detection_range: _set_state(State.CHASE)
	elif idle_timer <= 0: _set_state(State.PATROL)
...
func _chase(delta, dist):
	...
	if dist < attack_range: _set_state(State.ATTACK)
...
func _attack(delta, dist):
	attack_timer -= delta
	look_at(player.global_position, Vector3.UP)
	if dist > attack_range: _set_state(State.CHASE)
	elif attack_timer <= 0:
